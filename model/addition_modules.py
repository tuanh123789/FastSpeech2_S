...
class Reference_Encoder(nn.Module):
    """ Reference Encoder """
    def __init__(self, model_config):
        super(Reference_Encoder).__init__()
        self.idim = model_config["GST"]["idim"]
        self.conv_layers = model_config["GST"]["conv_layers"]
        self.conv_chans_list = model_config["GST"]["conv_chans_list"]
        self.conv_kernel_size = model_config["GST"]["conv_kernel_size"]
        self.conv_stride = model_config["GST"]["conv_stride"]
        self.gru_layers = model_config["GST"]["gru_layers"]
        self.gru_units = model_config["GST"]["gru_units"]
        self.convs = []
        self.padding = (self.conv_kernel_size - 1) // 2
        for i in range(self.conv_layers):
            self.conv_in_chans = 1 if i == 0 else self.conv_chans_list[i-1]
            self.conv_out_chans = self.conv_chans_list[i]
            self.convs += [
                nn.Conv2d(
                    self.conv_in_chans,
                    self.conv_out_chans,
                ...
                nn.BatchNorm(self.conv_out_chans),
                nn.ReLU(inplace=True)
            ]
        self.convs = nn.Sequential(*self.convs)

        self.gru_in_units = self.idim
        for i in range(self.conv_layers):
            self.gru_in_units = (
                self.gru_in_units - self.conv_kernel_size + 2 * self.padding
            ) // self.conv_stride + 1
        self.gru_in_units *= self.conv_out_chans
        self.gru = nn.GRU(self.gru_in_units, self.gru_units, self.gru_layers, batch_first=True)
